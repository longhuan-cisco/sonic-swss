...
- `PortsOrch::initGearboxPort()` at `portsorch.cpp:9585` (calls `sai_port_api->create_port()` at lines 9683, 9791)
- Uses `sai_port_api->create_port()` single variant for system-side and line-side gearbox ports
...
- **Written by:** sairedis library (in orchagent process), configured at `saihelper.cpp:344-386`
...
| handleSaiFailure | saihelper.cpp | 745-770 | Error handling with dump |
| OrchDaemon::flush | orchdaemon.cpp | 856-888 | Pipeline flush |
